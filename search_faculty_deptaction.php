...
if( isset($_POST["search"]) || isset($_POST["dept"]) ){
    ...
    $sql = "SELECT p.firstname, p.lastname, p.faculty, p.department, p.searchable, f.facultyname, d.departmentname FROM profile p INNER JOIN users u ON p.username = u.username INNER JOIN faculties f ON p.faculty = f.facultyid INNER JOIN departments d ON p.department = d.departmentid WHERE p.department = ? OR (p.firstname OR p.lastname LIKE ?)";
